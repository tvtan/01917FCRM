...
$y            = $pdf->getY();
 ...
 $info_right = 'Số tham chiếu: '.($invoice->rel_code?$invoice->rel_code:_l('dot_blank_code'));
//set so tham chieu
...
$pdf->writeHTMLCell(200, '', 150, 36, $info_right, 0, 0, false, true, ('L'), true);
$pdf->ln(12);


//Set detail
$pdf->SetFont($font_name, '', $font_size);
// var_dump($customer->company);die;
...
for ($i=0; $i < count($invoice->items) ; $i++) {
    ...
    $grand_total_paid=getTotalMoneyReceiveFromCustomer($invoice->id,'SO');

...
if($invoice->rel_type=='transfer')
{
    ...
    $strReceiver=mb_ucfirst($receiver->company,'UTF-8');
    if($invoice->is_staff==1)
    {
        $receiver=getStaff($invoice->receiver_id);
        $strReceiver=mb_ucfirst($receiver->fullname,'UTF-8');
    }
}
if($invoice->rel_type=='transfer')
{
...
<table width="100%" bgcolor="#fff" cellspacing="0" cellpadding="5" border="1px">
    <tr height="30" bgcolor="' . get_option('pdf_table_heading_color') . '" style="color:' . get_option('pdf_table_heading_text_color') . ';">
        ...
        <th scope="col"  width="17%" align="center">' . _l('Tên hàng hóa') . '</th>
...
for ($i=0; $i < count($invoice->items) ; $i++) {
    ...
    $tblhtml.='<td align="left">'.($invoice->items[$i]->note).'</td>';
    if($i==0)
    {
        $tblhtml.='<td >'.$invoice->items[$i]->reason.'</td>';    
    }
...
$certificate_root=($contract->code?$contract->code:$invoice->certificate_root);
$strmoney='<ul>';

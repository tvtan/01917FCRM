                                        ...
                                        <?php if($permission_condition['edit'] == true){
                                            ...
                                            <td class="text-center">
                                                ...
                                                foreach($custom_permission[$permission['shortname']]['permissions'] as $value) {
                                                    $statement = '';
                                                    if(isset($role)){
                                                        if(total_rows('tblrolepermissions',array('roleid'=>$role->roleid,'permissionid'=>$permission['permissionid'], "can_".$value =>1)) > 0){
                                                            $statement = 'checked';
                                                        }
                                                    }
                                                    if(has_rule($permission['shortname'],$member->staffid, $value)){?>
                                                    <div class="checkbox checkbox-danger">
                                                        <input type="checkbox" data-shortname="<?php echo $permission['shortname']; ?>" data-can-delete <?php echo $statement; ?> name="custompermission[<?=$value?>][]" value="<?php echo $permission['permissionid']; ?>">

            ...
            <div class="row">                
                ...
                <div class="col-xs-3 col-sm-3 col-md-3 col-lg-3 _buttons">
                    <div class="pull-right" style="display: none;">
                        <?php if( isset($quote) ) { ?>
                        <a href="<?php echo admin_url('sales/pdf/' . $quote->id . '?print=true') ?>" target="_blank" class="btn btn-default btn-with-tooltip" data-toggle="tooltip" title="" data-placement="bottom" data-original-title="In" ><i class="fa fa-print"></i></a>
            ...
            <?php echo form_open_multipart(admin_url('sales/sale_detail'), array('class' => 'sales-form', 'autocomplete' => 'off')); ?>
                <div class="row">
                  <div class="col-xs-3 col-sm-3 col-md-3 col-lg-3">            
                    ...
                    ?>
                    <?php $value = (isset($quote) ? $quote->prefix.$quote->code : '');?>
                    <?php echo render_input('rel_code','references',$value,'text',array('readonly'=>true)); ?>

                    <?php
                    $default_name = (isset($quote) ? $quote->id : NULL);
                    ...
                    <?php $value = (isset($quote) ? _d($quote->date_ht) : _d(date('Y-m-d')));?>
                    <?php echo render_date_input('account_date','account_date',$value,array(),array(),'hide'); ?>
                    ...
                    <?php
                    $selected=(isset($quote) ? $quote->address_id : get_staff_user_id());
                    ...
                    <!-- <div class="form-group">
                        <div class="radio radio-success radio-inline">
                            <input type="radio" name="isSingle" value="1" checked>
                            <label for="administrator"><?=_l('sale_single')?></label>
                        </div>
                        <div class="radio radio-success radio-inline">
                            <input type="radio" name="isSingle" value="0" <?=$quote->isSingle==0?'checked':''?>>
                            <label for="administrator"><?=_l('sale_vat')?></label>
                        </div>
                    </div> -->

                    <div class="checkbox checkbox-primary">
                        <input type="checkbox" value="0" <?=$quote->status_ck!=1?'checked':''?> name="status_ck" id="status_ck">
                    ...
                    <?php
                    $selected=(isset($quote) ? $quote->saler_id : get_staff_user_id());
                        if(empty($selected)) $selected=get_staff_user_id();
                ...
                <div class="col-xs-9 col-sm-9 col-md-9 col-lg-9">
                                            ...
                                            <tbody>
                                                ...
                                                if(isset($quote) && count($quote->items) > 0) {
                                                    foreach($quote->items as $value) {
                                                        ...
                                                        $class='';
                                                        $title='';
                                                        $exists_quantity=getProductQuantity($warehouse_id,$value->product_id);
                                                        ...
                                                        if($value->export_quantity!=$value->quantity)
                                                        {
                                                            ...
                                                            $tax=getUnitPrice($value->unit_cost,$value->tax_rate,false)*$max;
                                                ...
                                                <tr class="sortable item">
                                                    ...
                                                    ?>
                                                    <td><input style="width: 100px;<?=$type?>" class="mainQuantity <?=$class?>"  <?=$strmax?>  type="number" name="items[<?php echo $i; ?>][quantity]" value="<?php echo $max; ?>" <?='data-store="'.$data_store.'"'?> <?=$title?> <?=$quote->rel_id?'readonly':''?>></td>
                                                    ...
                                                    $transport_fee=$quote->adjustment;
                                        ...
                                        <table class="table text-right">
                                            <tbody>
                                                ...
                                                <tr>
                                                    ...
                                                    <td>
                                                        <div class="input-group">
                                                          <input type="number" name="discount_percent" id="discount_percent" min="0" class="form-control" placeholder="Phần trăm chiết khấu" aria-describedby="basic-addon2" value="<?=$quote->discount_percent?$quote->discount_percent:0?>">
                                            ...
                                            <tbody>
                                                ...
                                                <tr class="sortable item">
                                                    ...
                                                    <td><?php echo _format_number($max); ?>
                                                        <?php 
                                                        $tong_gv=0;
                                                        $quote_exports=getSaleProductDetail($value->product_id,$warehouse_id,$max);
                                                        foreach ($quote_exports as $key => $valP) {
                    ...
                    <?php (isset($quote)) ? $display='block' : $display='none' ?>
                            ...
                            <div class="col-md-4">
                                <div class="form-group mbot25">
                                    <select class="selectpicker no-margin" data-width="100%" id="custom_item_select_return" data-none-selected-text="<?php echo _l('add_item'); ?>" data-live-search="true">
                                        <option value=""></option>
                                        <?php foreach ($quote->items as $product) { ?>
                        ...
                        <div class="table-responsive s_table" style="overflow-x: auto;overflow-y: hidden;">
                            <table class="table items item-return no-mtop">
                                ...
                                <?php } ?>
                                    ...
                                    $j=0;
                                    $totalPriceR=0;
                                    if(isset($quote_returns) && count($quote_returns) > 0) {
                                        
                                        foreach($quote_returns as $value) {
                                            foreach ($quote->items as $key => $val) {
...
<script>
    ...
    var deleteTrItem = (trItem) => {
        ...
        totalPrice -= current.find('td:nth-child(6) > input').val() * current.find('td:nth-child(5)').text().replace(/\,/g, '');

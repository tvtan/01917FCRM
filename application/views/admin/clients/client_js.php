...
<script>
 ...
 $('.customer-form-submiter').on('click', function() {
   ...
   $('.dataTables_wrapper ').find('[name]').removeAttr('name');
   if (form.valid()) {
...
_table_api = initDataTable('.table-contracts-single-client', admin_url + 'contracts/init_contract_client/' + customer_id, [not_sortable_contracts], [not_sortable_contracts], 'undefined', [3, 'DESC']);
if(_table_api){
...
initDataTable('.table-sales-list', admin_url+'sales/get_sales_lead/'+customer_id, [1], [1],'');
...
initDataTable('.table-proposals-client-profile', admin_url+'quotes/init_client_quotes/'+customer_id, [1], [1],'');


initDataTable('.table-care_of', admin_url+'clients/init_client_care_of/'+customer_id, [1], [1],'');
...
if(_table_api) {
  _table_api.column(2).visible(false,false).columns.adjust();
    ...
    //  }
     ...
     $('body').on('hidden.bs.modal', '#address_modal', function() {
       $('#address_data').empty();
     });
...
function address(client_id, address_id) {
  var default_city  = 0;
  var default_state = 0;
  var default_ward  = 0;
 ...
 $.post(admin_url + 'clients/address/' + client_id + '/' + address_id).done(function(response) {
   $('#address_data').html(response);
   
   $('#address_modal').modal({show:true,backdrop:'static'});
   $('body').on('shown.bs.modal', '#address_modal', function() {
     var address_id = $(this).find('input[name="address_id"]').val();
     if (address_id == '') {
       $('#address_modal').find('input[name="address_id"]').focus();
     }
   });
   init_selectpicker();
   init_datepicker();
   ...
   $('#addressS_city').change((e)=>{

      var city_id = $(e.currentTarget).val();
      loadFromCityM(city_id, default_state, default_ward);
    });
   $('#addressS_state').change((e)=>{
      var state_id = $(e.currentTarget).val();
      loadFromStateM(state_id, e.currentTarget, default_ward);
    });
...
function validate_address_form() {
 _validate_form('#address-form', {
   addressS_country: 'required',
   addressS_city: 'required',
   addressS_area: 'required',
   // addressS_ward: 'required',
   addressS_state: 'required',
   
 }, addressFormHandler);
}

function addressFormHandler(form) {
 $('#address_modal input[name="is_primary"]').prop('disabled', false);
 var formURL = $(form).attr("action");
 var formData = $(form)[0];
 var formData = new FormData(formData);
 $.ajax({
   ...
   if ($.fn.DataTable.isDataTable('.table-address-list')) {
     $('.table-address-list').DataTable().ajax.reload();
   }
   if (response.proposal_warning && response.proposal_warning != false) {
     $('body').find('#address_proposal_warning').removeClass('hide');
     $('body').find('#address_update_proposals_emails').attr('data-original-email', response.original_email);
     $('#address_modal').animate({ scrollTop: 0 }, 800);
   } else {
...
var not_sortable_contacts = $('.table-address-list').find('th').length -1;
initDataTable('.table-address-list', admin_url + 'clients/addresses/' + customer_id, [not_sortable_contacts], [not_sortable_contacts]);
...
var loadFromCityM = (city_id, default_value_state, default_value_ward) => {
      var objState = $('#addressS_state');
      var objWard = $('#addressS_ward');
      ...
      if(city_id != 0 && city_id != '') {
        ...
        .done((data) => {          
          ...
          objState.selectpicker('refresh');
          if(foundSelected) {
            loadFromStateM(default_value_state, objState, default_value_ward);
          }
    ...
    var loadFromStateM = (state_id, currentTarget, default_value_ward) => {
      var objWard = $('#addressS_ward');
...
function ChangeClientCode(prefixClient='KH-',prefixClientOrg='TC-'){
  ...
  if(type==1)
  {
      newcode=prefixClient+code[1];
  }
  else
  {
      newcode=prefixClientOrg+code[1];
  }
  $('#code').val(newcode);
}
$('body').on('change', '#debt_limit_type', function(event, state) {
    var invoker = $(this).val();
    var debt_type_days=$('#debt_days');
    if (invoker == '2') {
       debt_type_days.addClass('fadeIn').removeClass('hide');
       return;
     } else {
       if (!debt_type_days.hasClass('hide')) {
         debt_type_days.removeClass('fadeIn').addClass('hide');
         debt_type_days.find('select').val(1).selectpicker('refresh');
       }
     }
  });
</script>
            ...
            <div class="row">
                ...
                <div class="col-xs-3 col-sm-3 col-md-3 col-lg-3 _buttons">
                    <div class="pull-right">
                        <?php if( isset($item) ) { ?>
                        <a href="<?php echo admin_url('sale_orders/pdf/' . $item->id . '?print=true') ?>" target="_blank" class="btn btn-default btn-with-tooltip" data-toggle="tooltip" title="" data-placement="bottom" data-original-title="In" aria-describedby="tooltip652034"><i class="fa fa-print"></i></a>
                        <a href="<?php echo admin_url('sale_orders/pdf/' . $item->id  ) ?>" class="btn btn-default btn-with-tooltip" data-toggle="tooltip" title="" data-placement="bottom" data-original-title="Xem PDF"><i class="fa fa-file-pdf-o"></i></a>
                        <?php } ?>
                    </div>
                </div>
            </div>
            ...
            <?php echo form_open_multipart($this->uri->uri_string(), array('class' => 'sales-form', 'autocomplete' => 'off')); ?>
                <div class="row">
                  <div class="col-xs-3 col-sm-3 col-md-3 col-lg-3">            
                    ...
                    if(empty($selected)) $selected=$customer_id;
                    ...
                    <?php if($item){ ?>
                    <?php $value = format_money(isset($item) ? getTotalMoneyReceiveFromCustomer($item->id,'PO') : 0);
                    $isDeposit=true;
                    ?>
                    <?php echo render_input('paid','paid_amount',$value,'text',array('readonly'=>true)); ?>
                    <?php if(lockDeposit($item->id)==false){ ?> 
                    <a href="#" onclick="receipt(<?=$item->customer_id; ?>,<?=$item->id; ?>,'PO',<?=$isDeposit; ?>); return false;" class="btn btn-info mbot25"><?=_l('new_receipt_deposit') ?></a>
                    <?php } } ?>
                     ...
                     <!-- <div class="form-group">
                        ...
                        <div class="radio radio-success radio-inline">
                            <input type="radio" name="isSingle" value="0" <?=$item->isSingle==0?'checked':''?>>
                    ...
                    <div class="checkbox checkbox-primary">
                        ...
                        <label for="status_ck" data-toggle="tooltip" data-original-title="" title=""><?=_l('hide_discount')?></label>
                    </div>
                                            ...
                                            <tbody>
                                                ...
                                                if(isset($item) && count($item->items) > 0) {
                                                    foreach($item->items as $value) {
                                                        $type='';
                                                        $class='';
                                                        if($value->quantity>$value->warehouse_type->product_quantity+$value->quantity)
                                                        {
                                                ...
                                                <tr class="sortable item">
                                                    ...
                                                    ?>
                                                    <td><input style="width: 100px;<?=$type?>" class="mainQuantity <?=$class?>"  <?=$strmax?>  type="number" name="items[<?php echo $i; ?>][quantity]" value="<?php echo $value->quantity; ?>" <?='data-store="'.($data_store+$value->quantity).'"'?> <?=$title?> <?=$item->rel_id?'readonly':''?>></td>
                            ...
                            <table class="table items item-return no-mtop">
                                ...
                                <tbody>
                                    <tr class="mains">
                                        ...
                                        <td>
                                            <input style="width: 100px " class="mainQuantity" type="number"
                                            value="1"  class="form-control" placeholder="<?php echo _l('item_quantity'); ?>">
                                        </td>
...
<?php init_tail(); ?>
<script>
    _validate_form($('.sales-form'),{code:'required',date:'required',customer_id:'required',saler_id:'required'});
    initClientDiscount();
    ...
    $(document).on('keyup', '.mainQuantity',(e)=>{
        ...
        if(parseInt(currentQuantityInput.val())<0)
        {
             currentQuantityInput.removeClass('error');
        }

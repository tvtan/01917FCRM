...
<script type="text/javascript">
    $(function(){
        ...
        initDataTable('.table-internals', window.location.href, [1], [1], filterList,[1,'DESC']);
        $.each(filterList, (filterIndex, filterItem) => {
            $('input' + filterItem).on('change', () => {
                $('.table-internals').DataTable().ajax.reload();
            });
        });
    });
    function var_status(status,id)
    {
        ...
        dataString={id:id,status:status,type: 'internals'};
        jQuery.ajax({
            type: "post",

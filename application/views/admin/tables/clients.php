...
foreach ($rResult as $r=> $aRow) {
    ...
    for ($i = 0; $i < count($aColumns); $i++) {
        ...
        else if ($aColumns[$i] == 'tblclients.address') {
            $_data = getClient($aRow['userid'],1);
        ...
        else if ($aColumns[$i] == 'mobilephone_number') {
            $_data="";
            $number=explode(',',$aRow['mobilephone_number']);
            foreach ($number as $value) {
                $_data.='<span class="label label-default mleft5 inline-block">'.$value.'</span>';
            }
        }
        ...
        if($aColumns[$i] == 'tblclients.create_by'){
            $_data = '<a href="' . admin_url('staff/profile/' . $aRow['tblclients.create_by']) . '">' . get_staff_full_name($aRow['tblclients.create_by']) . '</a>';
        }
        else if($i == 6) {
            //$_data = "x1";
        } else {

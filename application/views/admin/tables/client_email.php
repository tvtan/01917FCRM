...
$client_status=array(
    "1"=>"Khách hàng",
    "2"=>"Khách hàng được xác nhận chọn để chuyển thành khách hàng",
    "3"=>"Khách hàng chưa được xác nhận chọn để xác nhận"
);
...
foreach ($rResult as $aRow) {
    ...
    for ($i = 0; $i < count($aColumns); $i++) {
        ...
        if ($aColumns[$i] == '1') {
            $_data = '<div class="checkbox"><input type="checkbox" name="listid[]" value="' . $aRow['userid'] . '"><label></label></div>';
        } else if ($i ==4) {
            if ($_data != '') {
            ...
            $_data = '<a href="' . admin_url('clients/client/' . $aRow['userid']) . '">' . $aRow['code'] . '</a>';
            ...
            $_data .= '</div>';
        } else if($aColumns[$i]=='tblclients.status')
        {
            $_data='<span class="inline-block label label-'.get_status_label($aRow['tblclients.status']).'" task-status-table="'.$aRow['tblclients.status'].'">' . format_status_client($aRow['tblclients.status'],false,true).'';
            if(has_permission('invoices', '', 'view') && has_permission('invoices', '', 'view_own'))
            {
                if($aRow['tblclients.status']!=1){
                    $_data.='<a href="javacript:void(0)" onclick="var_status('.$aRow['tblclients.status'].','.$aRow['userid'].')">';
                }
                else
                {
                ...
                if($aRow['tblclients.status']==2) {
                    $_data .= '<a href="javacript:void(0)" onclick="var_status(' . $aRow['3'] . ',' . $aRow['userid'] . ')">';
                }
                else
                {
                    $_data .= '<a href="javacript:void(0)">';
            ...
            $_data.='<i class="fa fa-check task-icon task-finished-icon" data-toggle="tooltip" title="' . _l( $client_status[$aRow['tblclients.status']]) . '"></i>
                    </a>
                </span>';
        }
        else {

...
class Workplace_model extends CRM_Model
{
    private $statuses;
    ...
    {
        if (is_admin()) {
            $this->db->insert('tblworkplaces',$data);
    ...
    public function update_workplace($data_vestion,$id)
    {
        if (is_admin()) {
            // var_dump($data_vestion);die();
            $this->db->where('id',$id);
            $this->db->update('tblworkplaces',$data_vestion);
            if ($this->db->affected_rows() >0) {
                return true;
            }
            return false;
    ...
    {
        if (is_admin()) {
            $this->db->select('tblworkplaces.*');
            $this->db->where('tblworkplaces.id', $id);
            return $this->db->get('tblworkplaces')->row();
        }
    }

...
class Clients_model extends CRM_Model
{
    ...
    public function getClientReceivers($id = '', $where = array('tblclients.active' => 1))
    {
    ...
    public function getClientsCBO()
    {
        ...
        // <span class="label label-default mleft5 inline-block">' . $group . '</span>
        foreach ($result as $key => $customer) {
            $result[$key]['subtext']=getClientAddress($customer);
        }
    ...
    public function getClientAddress($customer_id = '', $where = array('type'=>'shipping'))
    {
        ...
        $this->db->order_by('is_primary', 'DESC');
        $result=$this->db->get('tbladdress')->result_array();
        foreach ($result as $key => $item) {
            $result[$key]['address']=getClientAddress2($customer_id,$item['id']);
            $result[$key]['city_name']=getProvince($item->city);
        }
    ...
    public function get_address($id, $single_result_type = 'row')
    {
        $this->db->where('id', $id);
        return $this->db->get('tbladdress')->$single_result_type();
    }
    /**
    ...
    public function update_address($data, $id, $client_request = false)
    {
        $hook_data['data'] = $data;
        $hook_data['id']   = $id;
        $hook_data         = do_action('before_update_addess', $hook_data);
        $data              = $hook_data['data'];
        $id                = $hook_data['id'];
        ...
        // Address cant change if is primary or not
        if ($client_request == true) {
            unset($data['is_primary']);
        }
        ...
        if ($this->db->affected_rows() > 0) {
            $affectedRows++;
            if (isset($data['is_primary']) && $data['is_primary'] == 1) {
                $this->db->where('user_id', $contact->userid);
                $this->db->where('id !=', $id);
        ...
        if ($affectedRows > 0) {
            logActivity('Address Updated [ID Adress:' . $id . ' ' . date('Y-m-d H:i:s') . ']');
            return true;
        }
        return false;
    }
    ...
    public function add_address($data, $customer_id, $not_manual_request = false)
    {
        ...
        $_data = do_action('before_create_address', $_data);
        $data  = $_data['data'];

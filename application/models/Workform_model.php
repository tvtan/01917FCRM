...
class Workform_model extends CRM_Model
{
    private $statuses;
    ...
    {
        if (is_admin()) {
            $this->db->insert('tblworkforms',$data);
    ...
    public function update_workform($data_vestion,$id)
    {
        if (is_admin()) {
            // var_dump($data_vestion);die();
            $this->db->where('workformid',$id);
            $this->db->update('tblworkforms',$data_vestion);
            if ($this->db->affected_rows() >0) {
                return true;
            }
            return false;
    ...
    {
        if (is_admin()) {
            $this->db->select('tblworkforms.*');
            $this->db->where('tblworkforms.workformid', $id);
            return $this->db->get('tblworkforms')->row();
        }
    }

...
class Position_model extends CRM_Model
{
    private $statuses;
    ...
    public function update_position($data_vestion,$id)
    {
        if (is_admin()) {
            $this->db->where('positionid',$id);
            $this->db->update('tblpositions',$data_vestion);
            if ($this->db->affected_rows() >0) {
                return true;
            }
            return false;

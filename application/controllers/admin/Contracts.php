...
class Contracts extends Admin_controller
{
    ...
    {
        if (!has_permission('hopdong', '', 'view') && !has_permission('hopdong', '', 'view_own')) {
            access_denied('hopdong');
        }
    ...
    public function contract($id = '')
    {
        if ($this->input->post()) {
            if ($id == '') {
                if (!has_permission('hopdong', '', 'create')) {
                    access_denied('hopdong');
                }
                $data=$this->input->post();
                
                $data['contract_value']=str_replace(',', '', $data['contract_value']);
                ...
                if (!has_permission('hopdong', '', 'edit')) {
                    access_denied('hopdong');
                }
    ...
    {
        if (!has_permission('hopdong', '', 'edit') && !has_permission('hopdong', '', 'create')) {
    ...
    {
        if (!has_permission('hopdong', '', 'create') && !has_permission('hopdong', '', 'edit')) {
            access_denied('hopdong');
        }
        if ($this->input->post()) {
            $data    = $this->input->post();
